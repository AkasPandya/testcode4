...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'fmnZ2Y6PGfmU9Wn',
      'isDeleted':false,
      'username':'Sabryna_Yundt',
      'email':'Golda.Cassin91@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.updateOne(User, { 'username':'Sabryna_Yundt' }, userToBeInserted,  { upsert: true });
    userToBeInserted = {
      'password':'NFsnRCal79IKIqD',
      'isDeleted':false,
      'username':'Abel2',
      'email':'Alberta44@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.updateOne(User, { 'username':'Abel2' }, userToBeInserted,  { upsert: true });
